...
static inline void psa_set_key_slot_number(
    ...
    psa_key_slot_number_t slot_number )
{
    attributes->core.flags |= IOTEX_PSA_KA_FLAG_HAS_SLOT_NUMBER;
 ...
 */
typedef struct iotex_psa_stats_s
{
 ...
 */
void iotex_psa_get_stats( iotex_psa_stats_t *stats );
 ...
 * * If the compile-time option IOTEX_PSA_INJECT_ENTROPY is enabled.
 *   Note that you must provide compatible implementations of
 *   iotex_nv_seed_read and iotex_nv_seed_write.
 * * In a client-server integration of PSA Cryptography, on the client side,
 *   if the server supports this feature.
 * \param[in] seed          Buffer containing the seed value to inject.
 * \param[in] seed_size     Size of the \p seed buffer.
 *                          The size of the seed in bytes must be greater
 *                          or equal to both #IOTEX_ENTROPY_BLOCK_SIZE
 *                          and the value of \c IOTEX_ENTROPY_MIN_PLATFORM
 *                          in `library/entropy_poll.h` in the Mbed TLS source
 *                          code.
 *                          It must be less or equal to
 *                          #IOTEX_ENTROPY_MAX_SEED_SIZE.
 *
 * \retval #PSA_SUCCESS
 ...
 */
psa_status_t iotex_psa_inject_entropy(const uint8_t *seed,
                                        size_t seed_size);
 ...
 */
static inline psa_ecc_family_t iotex_ecc_group_to_psa( iotex_ecp_group_id grpid,
                                                        size_t *bits )
{
    switch( grpid )
    {
        case IOTEX_ECP_DP_SECP192R1:
            *bits = 192;
            return( PSA_ECC_FAMILY_SECP_R1 );
        case IOTEX_ECP_DP_SECP224R1:
            *bits = 224;
            return( PSA_ECC_FAMILY_SECP_R1 );
        case IOTEX_ECP_DP_SECP256R1:
            *bits = 256;
            return( PSA_ECC_FAMILY_SECP_R1 );
        case IOTEX_ECP_DP_SECP384R1:
            *bits = 384;
            return( PSA_ECC_FAMILY_SECP_R1 );
        case IOTEX_ECP_DP_SECP521R1:
            *bits = 521;
            return( PSA_ECC_FAMILY_SECP_R1 );
        case IOTEX_ECP_DP_BP256R1:
            *bits = 256;
            return( PSA_ECC_FAMILY_BRAINPOOL_P_R1 );
        case IOTEX_ECP_DP_BP384R1:
            *bits = 384;
            return( PSA_ECC_FAMILY_BRAINPOOL_P_R1 );
        case IOTEX_ECP_DP_BP512R1:
            *bits = 512;
            return( PSA_ECC_FAMILY_BRAINPOOL_P_R1 );
        case IOTEX_ECP_DP_CURVE25519:
            *bits = 255;
            return( PSA_ECC_FAMILY_MONTGOMERY );
        case IOTEX_ECP_DP_SECP192K1:
            *bits = 192;
            return( PSA_ECC_FAMILY_SECP_K1 );
        case IOTEX_ECP_DP_SECP224K1:
            *bits = 224;
            return( PSA_ECC_FAMILY_SECP_K1 );
        case IOTEX_ECP_DP_SECP256K1:
            *bits = 256;
            return( PSA_ECC_FAMILY_SECP_K1 );
        case IOTEX_ECP_DP_CURVE448:
            *bits = 448;
            return( PSA_ECC_FAMILY_MONTGOMERY );
        default:
            *bits = 0;
 ...
 *                      (`IOTEX_ECP_DP_xxx`).
 * \return              #IOTEX_ECP_DP_NONE if \c curve is not recognized.
 * \return              #IOTEX_ECP_DP_NONE if \p bits is not
 *                      correct for \p curve.
 */
 ...
 *
 * When the compile-time option #IOTEX_PSA_CRYPTO_EXTERNAL_RNG is enabled,
 ...
 * implementation.
 *
 * The range of key identifiers from #IOTEX_PSA_KEY_ID_BUILTIN_MIN
 * to #IOTEX_PSA_KEY_ID_BUILTIN_MAX within the range from
 ...
 */
#define IOTEX_PSA_KEY_ID_BUILTIN_MIN          ((psa_key_id_t)0x7fff0000)

/** The maximum value for a key identifier that is built into the
 * implementation.
 *
 * See #IOTEX_PSA_KEY_ID_BUILTIN_MIN for more information.
 */
#define IOTEX_PSA_KEY_ID_BUILTIN_MAX          ((psa_key_id_t)0x7fffefff)
...
{
    return( ( key_id >= IOTEX_PSA_KEY_ID_BUILTIN_MIN ) &&
            ( key_id <= IOTEX_PSA_KEY_ID_BUILTIN_MAX ) );
}

/** Platform function to obtain the location and slot number of a built-in key.
 *
 * An application-specific implementation of this function must be provided if
 * #IOTEX_PSA_CRYPTO_BUILTIN_KEYS is enabled. This would typically be provided
 * as part of a platform's system image.
 *
 * #IOTEX_SVC_KEY_ID_GET_KEY_ID(\p key_id) needs to be in the range from
 * #IOTEX_PSA_KEY_ID_BUILTIN_MIN to #IOTEX_PSA_KEY_ID_BUILTIN_MAX.
 *
 * In a multi-application configuration
 * (\c IOTEX_PSA_CRYPTO_KEY_ID_ENCODES_OWNER is defined),
 * this function should check that #IOTEX_SVC_KEY_ID_GET_OWNER_ID(\p key_id)
 ...
 */
psa_status_t iotex_psa_platform_get_builtin_key(
    psa_key_id_t key_id,
