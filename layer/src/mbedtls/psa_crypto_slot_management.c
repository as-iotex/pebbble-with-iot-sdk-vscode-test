...
typedef struct
{
    psa_key_slot_t key_slots[IOTEX_PSA_KEY_SLOT_COUNT];
    unsigned key_slots_initialized : 1;
} psa_global_data_t;

static psa_global_data_t global_data;

int psa_is_valid_key_id( iotex_svc_key_id_t key, int vendor_ok )
{
    psa_key_id_t key_id = IOTEX_SVC_KEY_ID_GET_KEY_ID( key );
...
static psa_status_t psa_get_and_lock_key_slot_in_memory(
    iotex_svc_key_id_t key, psa_key_slot_t **p_slot )
{
    ...
    if( psa_key_id_is_volatile( key_id ) )
    {
         ...
         */
        status = iotex_svc_key_id_equal( key, slot->attr.id ) ?
    ...
    else
    {
        ...
        for( slot_idx = 0; slot_idx < IOTEX_PSA_KEY_SLOT_COUNT; slot_idx++ )
        {
            slot = &global_data.key_slots[ slot_idx ];
            if( iotex_svc_key_id_equal( key, slot->attr.id ) )
                break;
        }
        status = ( slot_idx < IOTEX_PSA_KEY_SLOT_COUNT ) ?
                 PSA_SUCCESS : PSA_ERROR_DOES_NOT_EXIST;
    }
...
static psa_status_t psa_load_builtin_key_into_slot( psa_key_slot_t *slot )
{
    ...
    if( ! psa_key_id_is_builtin(
            IOTEX_SVC_KEY_ID_GET_KEY_ID( slot->attr.id ) ) )
    {
        return( PSA_ERROR_DOES_NOT_EXIST );
    }

    /* Check the platform function to see whether this key actually exists */
    status = iotex_psa_platform_get_builtin_key(
...
psa_status_t psa_get_and_lock_key_slot( iotex_svc_key_id_t key,
                                        psa_key_slot_t **p_slot )
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    *p_slot = NULL;
...
#if defined(IOTEX_PSA_CRYPTO_STORAGE_C) || \
    defined(IOTEX_PSA_CRYPTO_BUILTIN_KEYS)
    ...
    return( status );
#else /* IOTEX_PSA_CRYPTO_STORAGE_C || IOTEX_PSA_CRYPTO_BUILTIN_KEYS */
    return( PSA_ERROR_INVALID_HANDLE );
#endif /* IOTEX_PSA_CRYPTO_STORAGE_C || IOTEX_PSA_CRYPTO_BUILTIN_KEYS */
}
...
psa_status_t psa_unlock_key_slot( psa_key_slot_t *slot )
{
    ...
    * IOTEX_TEST_HOOK_TEST_ASSERT that the lock counter is strictly greater
    * than zero: if the IOTEX_TEST_HOOKS configuration option is enabled and
...
psa_status_t psa_validate_key_persistence( psa_key_lifetime_t lifetime )
{
    ...
    else
    {
        ...
        if( PSA_KEY_LIFETIME_IS_READ_ONLY( lifetime ) )
            ...
            return( PSA_SUCCESS );
#else /* IOTEX_PSA_CRYPTO_STORAGE_C */
        return( PSA_ERROR_NOT_SUPPORTED );
#endif /* !IOTEX_PSA_CRYPTO_STORAGE_C */
    }
}

psa_status_t psa_open_key( iotex_svc_key_id_t key, psa_key_handle_t *handle )
{
#if defined(IOTEX_PSA_CRYPTO_STORAGE_C) || \
   defined(IOTEX_PSA_CRYPTO_BUILTIN_KEYS)
   psa_status_t status;
...
psa_status_t psa_purge_key( iotex_svc_key_id_t key )
{
   psa_status_t status;
   psa_key_slot_t *slot;
...
void iotex_psa_get_stats( iotex_psa_stats_t *stats )
{
   ...
   for( slot_idx = 0; slot_idx < IOTEX_PSA_KEY_SLOT_COUNT; slot_idx++ )
   {
       ...
       {
           psa_key_id_t id = IOTEX_SVC_KEY_ID_GET_KEY_ID( slot->attr.id );
...
#ifndef IOTEX_PSA_KEY_MANAGEMENT

#define IOTEX_KEY_SLOT_MAX  4
...
iotex_key_slot_t iotex_key_slot[IOTEX_KEY_SLOT_MAX] = {0};

psa_status_t iotex_get_and_lock_key_slot( iotex_svc_key_id_t key, uint8_t *key_data, size_t key_data_length, size_t *key_length)
{
    if (key >= IOTEX_KEY_SLOT_MAX)
      return (PSA_ERROR_INVALID_ARGUMENT);

    if (NULL == key_data)
      return (PSA_ERROR_INVALID_ARGUMENT);
    
    if (0 == key_data_length)
      return (PSA_ERROR_BUFFER_TOO_SMALL);

    if (iotex_key_slot[key].valid)
    {
        if (iotex_key_slot[key].key_length > key_data_length)
          return (PSA_ERROR_BUFFER_TOO_SMALL);

        memcpy(key_data, iotex_key_slot[key].key_data, iotex_key_slot[key].key_length);
        *key_length = iotex_key_slot[key].key_length;
    }
