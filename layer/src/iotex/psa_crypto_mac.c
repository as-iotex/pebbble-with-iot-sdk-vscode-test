...
#if defined(IOTEX_PSA_BUILTIN_ALG_HMAC)
static psa_status_t psa_hmac_abort_internal(
    iotex_psa_hmac_operation_t *hmac )
{
    iotex_platform_zeroize( hmac->opad, sizeof( hmac->opad ) );
...
cleanup:
    iotex_platform_zeroize( ipad, sizeof( ipad ) );
...
exit:
    iotex_platform_zeroize( tmp, hash_size );
    return( status );
}
#endif /* IOTEX_PSA_BUILTIN_ALG_HMAC */

#if defined(IOTEX_PSA_BUILTIN_ALG_CMAC)
static psa_status_t cmac_setup( iotex_psa_mac_operation_t *operation,
                                ...
                                const uint8_t *key_buffer )
{
    ...
    ret = iotex_cipher_setup( &operation->ctx.cmac, cipher_info );
    if( ret != 0 )
        goto exit;

    ret = iotex_cipher_cmac_starts( &operation->ctx.cmac,
                                      key_buffer,
                                      psa_get_key_bits( attributes ) );
exit:
...
#if defined(IOTEX_PSA_BUILTIN_ALG_HMAC) || \
    defined(IOTEX_PSA_BUILTIN_ALG_CMAC)

/* Initialize this driver's MAC operation structure. Once this function has been
 * called, iotex_psa_mac_abort can run and will do the right thing. */
    ...
    psa_algorithm_t alg )
{
    ...
    {
        iotex_cipher_init( &operation->ctx.cmac );
        status = PSA_SUCCESS;
    }
    else
#endif /* IOTEX_PSA_BUILTIN_ALG_CMAC */
...
psa_status_t iotex_psa_mac_abort( iotex_psa_mac_operation_t *operation )
{
    ...
    if( PSA_ALG_FULL_LENGTH_MAC( operation->alg ) == PSA_ALG_CMAC )
    {
        iotex_cipher_free( &operation->ctx.cmac );
    }
    else
#endif /* IOTEX_PSA_BUILTIN_ALG_CMAC */
#if defined(IOTEX_PSA_BUILTIN_ALG_HMAC)
...
static psa_status_t psa_mac_setup( iotex_psa_mac_operation_t *operation,
                                   const psa_key_attributes_t *attributes,
                                   const uint8_t *key_buffer,
    ...
    size_t input_length )
{
    ...
    if( PSA_ALG_FULL_LENGTH_MAC( operation->alg ) == PSA_ALG_CMAC )
    {
        return( iotex_to_psa_error(
                    iotex_cipher_cmac_update( &operation->ctx.cmac,
                                                input, input_length ) ) );
    }
    else
#endif /* IOTEX_PSA_BUILTIN_ALG_CMAC */
    ...
    uint8_t *mac, size_t mac_size )
{
    ...
    if( PSA_ALG_FULL_LENGTH_MAC( operation->alg ) == PSA_ALG_CMAC )
    {
        ...
        int ret = iotex_cipher_cmac_finish( &operation->ctx.cmac, tmp );
        if( ret == 0 )
            memcpy( mac, tmp, mac_size );
        iotex_platform_zeroize( tmp, sizeof( tmp ) );
        return( iotex_to_psa_error( ret ) );
    }
    else
#endif /* IOTEX_PSA_BUILTIN_ALG_CMAC */
    ...
    size_t mac_length )
{
    ...
    if( iotex_psa_safer_memcmp( mac, actual_mac, mac_length ) != 0 )
...
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    iotex_psa_mac_operation_t operation = IOTEX_PSA_MAC_OPERATION_INIT;
    ...
    if( input_length > 0 )
    {
        status = iotex_psa_mac_update( &operation, input, input_length );
        if( status != PSA_SUCCESS )
            goto exit;
    }
...
#endif /* IOTEX_PSA_BUILTIN_ALG_HMAC || IOTEX_PSA_BUILTIN_ALG_CMAC */
